...
| DET-1 | histogram bins fold batch by batch (`histogram_fold`); an element with fewer than N strictly larger keys in the whole list has fewer than N in any batch, so top-N compaction drops nothing (`topN_of_superset`; ties not modelled) | `Nice.topN_of_superset` | `common/src/distribution_stats.rs`, `common/src/number_stats.rs` | tests | proved | 5 |
