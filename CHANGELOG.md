...
- Detailed claims can now use an index. `check_level <= 1` is emitted as a literal so Postgres can match the new partial index `idx_fields_cl1_id`; against a bound parameter it cannot prove predicate implication and walks the primary key instead (measured 93ms → 21ms on a 1.2M row fixture, generic plan). Claimability is likewise spelled `last_claim_time IS NULL OR last_claim_time <= $1` rather than `COALESCE(last_claim_time, 'epoch') <= $1`, which no btree index can answer.
...
- New migration: `schema/migrations/2026-08-17_detailed_claim_indexes.sql`.
