...
- Make the scheduled jobs incremental ([#98](https://github.com/wasabipesto/nice/issues/98)-adjacent; the 6-hourly run was taking ~80 minutes at 40-70GB RSS and causing an hour-long window of API 503s every run). A new one-row `job_state` table stores the highest processed submission id; each run evaluates consensus only for fields with new detailed submissions and recomputes statistics only for chunks whose fields received any submission, then advances the watermark (held back by a safety margin, since `MAX(id)` is not a transactionally safe boundary). Bases with no new submissions are skipped entirely.
- Stream the downsampling aggregation. Chunk and base statistics now fetch canon submissions one chunk at a time and fold them through accumulators (`DistributionAccumulator`, `NumbersAccumulator` - provably batch-order-independent, with unit tests), instead of loading a whole base's canon submissions in one query: 3.5M+ rows for the largest base, tens of GB once the jsonb decoded, then cloned again for base-level stats. Peak memory now tracks the largest chunk (~tens of MB).
- Add `--full` / `just jobs-full`: the previous sweep-everything behavior. Manual changes that create no submission - disqualifying one, in particular - are invisible to the watermark, so run the full sweep after making them; it re-evaluates every field with detailed submissions and recomputes every chunk and base. Verified end-to-end: disqualifying a canon submission propagates through field reset, chunk stats, chunk/base distributions and the top-numbers lists.
- Jobs logging is now one line per change plus per-base summaries, instead of one line per field and per chunk.
- New migration: `schema/migrations/2026-08-20_job_state.sql` (initializes the watermark to the current max submission id; run `just jobs-full` once after deploying if stats might be stale).

- Make queue refills single-flight and run them on the requesting handler's database connection. Previously every request that observed a low queue launched its own bulk claim — under fleet load 4-6 identical refills landed together (the niceonly queue was observed at 1,226 against the 250 one refill can reach) — and each refill checked out a second pool connection while its request held the first. With the pool at its default size of 10, that combination was measured saturating the pool in 10-20s stretches about once a minute, fast-failing unrelated requests (including `/submit`) into 5s-timeout 503s at ~20/minute. Now a per-queue gate admits one refill at a time (contenders skip straight to popping, re-checking under the gate so a just-completed refill isn't repeated), and refills borrow the caller's connection instead of a second one. Brief empty-queue windows while a refill is in flight fall through to the direct claim paths, which are all chunk-scoped or indexed since #100/#102 and cost milliseconds.

- Scope the detailed `Next` claim (and the `Next`-based recheck) to the frontier chunk. The unscoped form walked the primary key from id 1 and filtered every completed field below the frontier on every request — 1.55s and ~1.6GB of buffer reads per claim in production, for the ~20% of detailed claims that don't use `Thin`. The frontier chunk is found via `chunks.minimum_cl`, which is sound because it shares a writer with the field `check_level`s it summarizes (both advance only when `jobs` runs), and a stale value errs in the safe direction. Semantics are unchanged — chunks partition fields in id order, so the first `minimum_cl <= N` chunk with a claimable field contains the globally-first claimable field. Measured on production under `force_generic_plan`: 1,547ms → 32ms. Nice-only `Next` (cl=0) keeps the unscoped form, which the `idx_fields_cl0_id` partial index already serves in under a millisecond.

- Fix `/claim/detailed` timing out for most requests ([#98](https://github.com/wasabipesto/nice/issues/98)). The frontier chunk is now the first under-explored chunk that still holds a *claimable* field, rather than simply the first under-explored one: once every field in the head chunk carried a live claim — the steady state under fleet load, and one the chunk could not exit on its own because `checked_detailed` counts completed work and is only written by `jobs` — every detailed claim found nothing and fell through to a scan of the whole `fields` table. Thanks to [Janzert](https://github.com/Janzert) for the report and the diagnosis.
- Claimability is spelled `last_claim_time IS NULL OR last_claim_time <= $1` rather than `COALESCE(last_claim_time, 'epoch') <= $1`, which no btree index can answer, and `check_level <= 1` is emitted as a literal so a partial index could be matched (against a bound parameter Postgres cannot prove predicate implication). No such index is created — see below.
- Remove the claim fallback that re-issued fields another client was still working on. When every candidate is claimed, `/claim/detailed` now returns 503 instead of running a second pass with `maximum_timestamp = NOW()` that matched every row regardless of holder — which under a saturated chunk handed the same few fields to every client that asked, and restamped `last_claim_time` on each re-issue so the original claim never aged out.
...
- No new indexes, after measuring the two proposed ones against production rather than a fixture. `idx_fields_chunk_claim_cover` (est. ~26GB) was to make the frontier chunk probe an index-only scan; the probe costs 35ms per queue refill without it, which is 0.35ms per field claimed. `idx_fields_cl1_id` (est. ~7GB) was to let detailed claims skip completed work, but `check_level <= 1` covers 95.6% of a 130M-row `fields` table in production, so the index would be nearly the size of the primary key while excluding almost nothing. The frontier-chunk fix above needs neither.
- Use a single `EXISTS` for the frontier chunk's claimability test rather than splitting it across the `IS NULL`/expired disjunction. The split form is faster given an index covering all four predicate columns, and slower without one: Postgres evaluates the halves in order, so on a picked-over chunk the `IS NULL` half scans the whole chunk to conclude nothing before the expired half finds its row. Measured against production: split 78.0ms, single 35.2ms (field probe 48ms vs 0.24ms).

- Serve `/estimate` from an in-memory benchmark corpus cache instead of re-fetching and re-decoding up to 2000 jsonb reports per request (~190ms and a pool connection held per call, ~100 calls per fleet-controller tick). The corpus refreshes at most once per TTL (60s default, `ESTIMATE_CACHE_TTL_SECS`), is invalidated immediately when `/benchmark` stores a new report, and serves stale during a failed refresh — one bounded retry per TTL window, so a database outage degrades `/estimate` to slightly-stale answers rather than errors. Responses are unchanged (verified byte-identical against the previous implementation across all matching stages).
- Harden the database pool against worker starvation: the connection checkout timeout drops from r2d2's 30s default to 5s (`DATABASE_POOL_TIMEOUT_SECS`), and API handlers now return a fast 503 (`service_unavailable`) when no connection is available instead of panicking after pinning a request worker for 30s. Field-queue refills likewise degrade to their direct-claim fallback instead of panicking.
