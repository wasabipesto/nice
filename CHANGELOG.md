...
- Add `proofs/`, a Lean 4 + Mathlib project that states and proves the mathematics the search relies on (design and phases in `proofs/DESIGN.md`). Phase 0: definitions (`IsNice`, positional digits, the residue filter), the first proved claims (digit-count necessity, the digit-sum congruence, residue-filter soundness, empty-filter nonexistence, base 11 dead, 69 nice in base 10 by `decide`), the claims registry `proofs/CLAIMS.md`, `Lean:` tags at four Rust sites, and `just lean-build` / `just lean-claims` / `just lean-fixtures` / `just lean-conformance`. Phases 1–4 prove the base range and its numeric cutoffs, the residue/LSD/stride/seeded-check models, the MSD interval-domain filter with Hall soundness and the recursive subdivision, the cross-end certificate, and the end-to-end theorem `niceonly_complete` / `niceonly_sound` for the modelled niceonly pipeline; `Refuted.lean` records the 2026-02 skip with its witness. A `Lean proofs` CI job (`.github/workflows/lean.yml`) builds the library, runs the conformance check against the Rust-emitted fixtures, and validates the claims registry, on changes under `proofs/`.
...
- Take `cubecl-spirv` out of the `gpu` umbrella. On macOS it needs the Vulkan SDK (`VULKAN_SDK` set) just to build, for a compiler that never runs there, which made `--features gpu` fail on a stock mac. `gpu` now builds on every platform with no GPU libraries installed. `cubecl-spirv` is opt-in (`--features gpu,cubecl-spirv`) and the `-gpu` docker image is still built with it, so Linux Vulkan users keep plane-scoped compaction.
- Fix `--features vulkan` on its own not compiling: the client's queued-field path was gated on every GPU feature, so with only Vulkan it was a match whose single arm was `unreachable!`, with no type to infer (E0282). It is now gated on the backends that queue.
- Under `--gpu-backend auto`, a CUDA-family backend that is skipped because the NVIDIA driver loaded but the CUDA toolkit (NVRTC) is missing now logs a warning saying so, since that machine could be running the faster backend. A machine with no NVIDIA driver at all still falls through quietly. The fatal "no usable GPU backend" message now also lists the compiled-in shader compilers (`cubecl-spirv`, `cubecl-metal`).
- Print warnings up front about specific feature combinations: `cubecl-hip` without `hipconfig` on PATH and `openssl-tls` together with `rustls-tls`.
...
- Let the benchmark warm up the MSD tuning for three seconds before locking in a single value for the actual run
- Start the MSD filter at 1/64 of the field size instead of a hardcoded chunk value in order to decrease the amount of upfront cost when CPU is relatively weak and invalid ranges are large
- Report the real GPU name when the `cubecl-cuda` backend is in use (previously a placeholder `cubecl-cuda device 0`, now uses`cuDeviceGetName`)
- Serve the detailed `Next` claim strategy from pre-claimed in-memory queues, like `Thin` and niceonly already were. Only `Random` (1%) still claims directly. `/status` reports the new queue sizes.
- Add uptime, pool status and build version to the status endpoint
