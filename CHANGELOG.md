...
- Fix `--features vulkan` on its own not compiling: the client's queued-field path was gated on every GPU feature, so with only Vulkan (which never queues) it was a match whose single arm was `unreachable!`, with no type to infer (E0282). It is now gated on the backends that queue. Every CI job paired `vulkan` with `cubecl`, which hid this; the lint recipe and the documented-deps workflow now build it alone.
