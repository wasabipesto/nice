...
- Benchmark: the niceonly GPU sweep now pins the MSD floor (at the adaptive controller's cap) instead of letting it adapt per field. The floor is process-global state that moved hundreds of times within one 60-second sweep, so every scenario's rate depended on where the previous scenarios had left it and results were not comparable across machines (an A100 scored below an RTX 3060). `NICE_GPU_MSD_FLOOR` still overrides it for floor sweeps. The report's `config` gains `gpu_msd_floor`.
