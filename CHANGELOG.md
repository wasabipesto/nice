...
- Build the Linux-x86_64 and Linux-aarch64 release binaries inside PyPA's `manylinux_2_28` containers (glibc 2.28, current gcc) instead of on the GitHub runner, so they run on hosts as old as glibc 2.28 (the 3.4.1 binaries required 2.38); CI asserts the floor on the artifacts.
