...
fn private_to_public(p: SubmissionPrivate) -> Result<SubmissionRecord, String> {
    ...
    Ok(SubmissionRecord {
        ...
        elapsed_secs: i32_to_u32(p.elapsed_secs)?,
        username: p.username,
        user_ip: p.user_ip,
...
fn public_to_private(p: SubmissionRecord) -> Result<SubmissionPrivate, String> {
    ...
    Ok(SubmissionPrivate {
        ...
        submit_time: p.submit_time,
        elapsed_secs: u32_to_i32(p.elapsed_secs)?,
        username: p.username,
        user_ip: p.user_ip,
        client_version: p.client_version,
        disqualified: p.disqualified,
...
fn build_new_row(
    ...
    Ok(SubmissionPrivateNew {
        ...
        elapsed_secs: (Utc::now() - claim_record.claim_time).num_seconds() as i32, // i64 -> i32 is safe
