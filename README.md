...
- In order to build the client with GPU acceleration, enable the `nice_client/gpu` feature. It is an umbrella over multiple backends below:
  ...
  - `nice_client/cubecl-spirv` and `nice_client/cubecl-metal` switch the `cubecl` backend's shader compiler from naga (WGSL) to CubeCL's own SPIR-V or MSL codegen on Vulkan or Metal respectively. `cubecl-spirv` is part of the `gpu` umbrella (it pulls in `ash`); `cubecl-metal` is experimental and opt-in. Results are identical (device parity tests cover both); `NICE_CUBECL_WIDE=1` additionally opts into the 64-bit chunk scan on devices that expose `u64`, which is slower on every wgpu device measured so far.
    - **On macOS `cubecl-spirv` needs the [Vulkan SDK](https://vulkan.lunarg.com/sdk/home#mac) at build time**, and since the `gpu` umbrella includes it, `--features gpu` does not build on a stock macOS machine. `cubecl-wgpu`'s build script panics whenever the target is macOS, the `spirv` feature is on, and `VULKAN_SDK` is unset. The check is on the variable alone, not on anything inside it, and `ash` is built in `loaded` mode so nothing links against the loader — pointing `VULKAN_SDK` at an existing Vulkan install is therefore enough to get past the panic. `brew install molten-vk vulkan-loader` on its own is *not* enough, because Homebrew never sets `VULKAN_SDK`. If you don't need SPIR-V, `--features cubecl` builds on macOS with nothing installed.
...
Most backends need nothing installed to *build* — they load their driver at runtime — so the "needs to build" column is empty for all but two. Both exceptions fail in ways that don't name the missing dependency, so check here before debugging a link error.
...
| `cubecl-spirv` (in the `gpu` umbrella) | Vulkan devices, via the `cubecl` backend | nothing on Linux/Windows; **on macOS the Vulkan SDK, with `VULKAN_SDK` set** | same as `cubecl` |
